 ...
 * @param isHorizontal 
 */
export function nonLayeredTidyTreeAlgorithms(root, isHorizontal) {
    // do layout
    AlgorithmUtil.layer(root, isHorizontal);
    const wt: WrappedTidyTree = WrappedTidyTree.fromNode(root, isHorizontal);
    // console.log(wt)
    firstWalk(wt);
    secondWalk(wt, 0);
...
function firstWalk(t: WrappedTidyTree) {
    if (t.cs === 0) {
        setExtremes(t);
        return;
    }
...
function setExtremes(t: WrappedTidyTree) {
    if (t.cs === 0) {
        t.el = t;
        t.er = t;
        t.msel = t.mser = 0;
...
function moveSubtree(t: WrappedTidyTree, i: number, si, dist) {
    t.c[i].mod += dist;
    t.c[i].msel += dist;
    t.c[i].mser += dist;
...
function nextLeftContour(t: WrappedTidyTree) {
    return t.cs === 0 ? t.tl : t.c[0];
}

function nextRightContour(t: WrappedTidyTree) {
    return t.cs === 0 ? t.tr : t.c[t.cs - 1];
}

function bottom(t: WrappedTidyTree) {
    return t.y + t.h;
}
